...
**更新** 新增的固定长度字节数组类型，专门用于处理超出 u64 范围的十进制数组（Decimal128/256）。

- 适用范围：Decimal128/256，且值超出 u64 范围。
- 编码策略：字典去重（按原始字节表示）→ UInt16 索引（BitPackedArray）+ FSST 压缩字典值。
- 内存布局：IPC 头部（逻辑类型=FixedLenByteArray，物理类型=UInt16）+ FixedLenByteArrayHeader（键大小、值大小、箭头类型、精度、刻度等）+ BitPackedArray（键）+ 对齐 + FSST 值段（符号表 + 原始字节数 + 压缩大小 + 压缩数据 + CompactOffsets）。
...
- 固定长度字节数组往返测试：[test_roundtrip.cpp:271-324](file://tests/test_roundtrip.cpp#L271-L324)
